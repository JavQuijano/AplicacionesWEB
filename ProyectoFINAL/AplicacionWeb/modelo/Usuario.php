...
class Usuario extends EntidadBase
{
    ...
    public function actualizarEstado($estado){
        $query = "update usuarios set estado = '$estado' where clvUsuarios = '$this->ClvUsuarios';";
        $this->runQuery($query);
    }

    public function getUsuariosBD(){
        $query = "select clvusuarios, nombreusuario, pregunta, respuesta, estado 
                  from usuarios join preguntasseguridad on clvusuarios = usuarios_clvusuarios;";
    ...
    public function updateInfo($data){
        try {
            $this->ClvUsuarios = $data['ClvUsuarios'];
            $this->updateRespuesta($data['ClvUsuarios'], $data['Respuesta']);
            if(isset($data['Estado'])){
                $estado = $this->verificarEstado($data['Estado']);
    ...
    public function updateRespuesta($clvUsuario, $respuesta){
        $query = "update preguntasseguridad set respuesta = '$respuesta'
                  where usuarios_clvusuarios = '$clvUsuario';";
        $this->runQuery($query);
    }
    ...
    public function reiniciarIntentos(){
        $query = "update acceso set intentos = 0
                  where usuarios_clvusuarios = '$this->ClvUsuarios';";
        $this->runQuery($query);
    }
